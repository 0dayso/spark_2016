...
object Sample { 
 ...
 //输入是字符串表示的ip，输出是该ip对应的整数值 
  ...
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("analyse").setMaster("local")
   ...
   // val lines = sc.textFile("file:///xujing/06171511-5")
   // val ips = sc.textFile("file:///xujing/ip.txt")
    ...
    val ss = lines.map(_.split("\\s+",-1)).cache()
    println("原始数据：******************************************************************************")
   // ss.take(10).foreach(p=>println(p(0)+" "+p(1)+" "+p(2)+" "+p(3)+" "+p(4)+" "+p(5)+" "+p(6)))
    
    //提取出域名和ttl,ip "0.0.0.0"
    val domain_ttl_ip=ss.map{p => (p(3),(p(5),p(6)))}.reduceByKey((a,b) => (a._1 + ","+b._1,a._2+","+b._2)).
    map(p => (p._1,p._2._1.split(',').map(p =>changeDouble(p)),p._2._2.split(',').map(a =>changeIp1(a)))).cache()
    println("域名和ttl:*****************************************************************************")
    domain_ttl_ip.take(10).foreach(p=> println("提取出域名和ttl_ip:	"+p._1+" "+p._2.mkString(" ")+" "+p._3.mkString(" ")))   
    
    //域名对应 数字比,ttl均值，个数，变化数，标准差,ip数,国家数
    val domains7=domain_ttl_ip.map{p =>(p._1,(digit(p._1),average(p._2),numTtl(p._2),change(p._2),deviation(p._2),numIp(p._3),countries(p._3,localdata1)))}.cache()    
    println("域名,(数字比,均值，个数，变化数，标准差,ip数,国家数):**********************************************")
    domains7.take(10).foreach(p=> println("域名和7个属性：	"+p._1+" "+p._2._1+" "+ p._2._2+" "+ p._2._3+" "+ p._2._4+" "+ p._2._5+" "+ p._2._6+" "+ p._2._7))
