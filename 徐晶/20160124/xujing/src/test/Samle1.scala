 ...
 //输入是字符串表示的ip，输出是该ip对应的整数值 
  ...
  def main(args: Array[String]) {
   ...
   domain_type_ttl_ip.take(10).foreach(p=> println("提取出域名，类别和ttl_ip:	"+p._1+" "+p._2+" "+p._3.mkString(" ")+" "+p._4.mkString(" ")))   
    ...
    val domains7=domain_type_ttl_ip.map{p =>(p._1+" "+p._2+" "+"1:"+digit(p._1)+" "+"2:"+average(p._3)+" "+"3:"+numTtl(p._3)+" "+"4:"+change(p._3)+" "+"5:"+deviation(p._3)+" "+"6:"+numIp(p._4)+" "+"7:"+countries(p._4,localdata1))}    
